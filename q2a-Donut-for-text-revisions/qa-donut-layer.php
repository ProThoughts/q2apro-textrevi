<?php
    ...
    class qa_html_theme extends qa_html_theme_base
    {
        ...
        function head_script() // change style of WYSIWYG editor to match theme better
        {
			...
			if($this->template=='ask' || $this->template=='question')
			{
				...
				$(document).ready(function()
				{
					...
					if($('.qa-template-ask  #wmd-input-content').length > 0)
					{
						$('<span id=\"requireMoreChars\" style=\"display:block;color:#FF9000;\"></span>').insertAfter('#wmd-input-content');

						// set text in markdown editor
						$('.qa-template-ask  #wmd-input-content').html('Describe the context of the text in one sentence. Help us to understand what the purpose is.&#13;&#10;&#13;&#10;***Text Start***&#13;&#10;&#13;&#10;Write your text here... do not remove the start and end text markers!&#13;&#10;&#13;&#10;***Text End***');
					}
					// display necessary number of characters
					$('.qa-template-ask #wmd-input-content').keyup(function(e)
					{
                              if($(\"#wmd-input-content\").val().length > 0)
                              {
     						// char count
     						// var nchars = (25 - $(\"#wmd-input-content\").val().length);
     						var nchars = (25 - $(\"#wmd-input-content\").val().length);
     						var wordcount = $(\"#wmd-input-content\").val().match(/\S+/g).length;

     						$('#requireMoreChars').html(wordcount+' words (max. 500 allowed)');
     						if(wordcount>500)
     						{
     							$('#requireMoreChars').html('<span style=\"color:#F00;\">'+wordcount+' words - too many!</span>');
     							// $(\"#wmd-input-content\").val( $(\"#wmd-input-content\").val().substring(1,500) );
     							// http://stackoverflow.com/questions/17909646/counting-and-limiting-words-in-a-textarea
     						}
                              }
					});

					// submit on enter
					// $('#wmd-input-content, #wmd-input-q_content').keydown(function(e) {
					...
					$(window).bind('beforeunload', function() {
						if(warn_on_leave) {
							return 'Your text has not been saved. All inputs are lost if you leave the page!';
						}
					});
			...
			if($this->template=='question')
			{
				$this->output('
					<script type="text/javascript">
						$(document).ready(function()
						{
							var copy = $(".qa-q-view-content div[itemprop=\'text\']").text();
							$("#wmd-input-a_content").text(copy);
						});
					</script>
				');
			}
		...
		public function head_title()
		{
			...
			{
				// tag represents language
				$tag = $this->content['q_view']['raw']['tags'];
				$headtitle = ucfirst($tag).': '.$headtitle;
			}
       ...
       function page_title_error()
		...
		if(isset($this->content['q_view']['raw']['tags']))
		{
			...
			$this->output('
                    <p style="margin:5px 0;">
                         <a style="color:#77A;" href="'.qa_path('tag').'/'.$tag.'">Language: '.ucfirst($tag).'</a>
                    </p>
               ');
		}
        ...
        {
			// output no best answer select - disabled
        ...
        public function donut_site_header()
        {
			...
			if($this->template == 'questions' || $this->template == 'question' || $this->template == 'qa')
			{
				$this->output('
				<div class="container rv_intro_container">
					<div class="row">
						<p>
							<span class="fa fa-heart"></span> Hello everyone, you can <a href="/ask">post your text here</a> and our community of native speakers will correct it.
						</p>
					</div>
				</div>
				...
				.rv_intro_container a {
					color:#FFF;
					border-bottom:1px dotted #FFF;
					border-bottom:1px dotted rgba(255,255,255,0.5);
				}
				.rv_intro_container a:hover {
					text-decoration:none;
					border-bottom:1px dotted #FFF;
					border-bottom:1px dotted rgba(255,255,255,1);
				}
				.rv_intro_container p {
					padding:0;
					margin:0;
					font-size:15px;
				}
        ...
        private function donut_attribution()
        {
			...
			*/
            $this->output(
                    '<div class="qa-attribution">',
                    'Theme by <a href="http://amiyasahu.com">Amiya Sahu</a>',
                    '</div>'
            );
        }
		...
		function initialize()
		{
			...
			if(isset($hash))
			{
				...
				$hash = array('ask' => $hash['ask']) +
						array('unanswered' => $hash['unanswered']) +
						array('questions' => $hash['questions']) +
						$hash;
			...
			// change link from users to monatsbeste
			// $this->content['navigation']['main']['user']['url'] = qa_path('bestusers');
			...
			*/
			...
			{
				$this->content['error'] = 'We do not answer to questions that were already answered in the <a href="'.qa_path('faq').'">Frequently Asked Questions</a> section. In case you do not receive an email from us within 48 hours, you will find the answer there.';
			}

			// QUESTION page
			if($this->template=='question')
			{
				// can go into userrules
